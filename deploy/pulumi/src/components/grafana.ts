    ...
    'cerus-grafana-datasources',
    {
      ...
      stringData: {
        'datasources.yml': [
          ...
          '   type: prometheus',
          '   orgId: 1',
          `   url: http://cerus-prometheus-kube-prom-prometheus.${config.namespace}.svc.cluster.local:9090`,
          '   basicAuth: false',
          '   isDefault: true',
          '   editable: false',
          ' - name: ClickHouse',
          ...
          '   jsonData:',
          '    defaultDatabase: plausible_events_db',
          '    port: 9000',
          `    server: cerus-analytics-clickhouse.${config.namespace}.svc.cluster.local`,
        ].join('\n'),
      },
    },
    { provider, dependsOn }
